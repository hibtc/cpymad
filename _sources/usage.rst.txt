...
argument that will be forwarded as input to the MAD-X interpretor. For
example:
...
state of the MAD-X interpretor state go through the
...
- a command to be parsed by the MAD-X interpretor
- to use a file on disk or the network
- output data to be parsed on the python side
- to potentially modify the MAD-X interpretor state by executing a command
...
Evaluate an expression in the MAD-X interpretor:
...
The output of the MAD-X interpretor can be controlled using the ``redirect``
